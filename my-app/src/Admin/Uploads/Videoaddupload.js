  ...
  const handleLinkSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/videos/upload-link', {
        ...
        body: JSON.stringify({ videoLink, title, description, status: status === 'active', displayAt, type, subcategory }),
      });
      ...
      <div className="d-flex justify-content-center align-items-center vh-70 bg-light">
        <div className="container1 p-6 rounded shadow" style={{ maxWidth: '700px' }}>
          ...
          <div className="mb-3">
            ...
            <input type="text" id="type" className="form-control" placeholder="Enter video type" value={type} onChange={(e) => setType(e.target.value)} />
          </div>
          <div className="mb-3">
            <label htmlFor="subcategory" className="form-label fw-bold">Subcategory</label>
            <input type="text" id="subcategory" className="form-control" placeholder="Enter video subcategory" value={subcategory} onChange={(e) => setSubcategory(e.target.value)} />
          </div>
          <div className="mb-3">
